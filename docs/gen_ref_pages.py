...
# Generate CSS from chunkload.utils.colors.Palette
try:
    from chunkload.utils.colors import Palette as _CLPalette

    pal = _CLPalette()
    sem = pal.semantic_colors()

    css = f"""/* Generated from chunkload.utils.colors.Palette */
:root {{
  --cl-white: {pal.white};
  --cl-black: {pal.black};
  --cl-gray: {pal.gray};
...
# New: recurse under 'chunkload' and skip FastAPI runtime modules ('chunkload/api/*')
pkg_root = src / "chunkload"
for path in sorted(pkg_root.rglob("*.py")):
    rel = path.relative_to(src)
    parts = rel.parts
    if len(parts) >= 2 and parts[0] == "chunkload" and parts[1] == "api":
        continue
