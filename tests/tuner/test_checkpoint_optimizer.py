...
def test_one_log_no_violations():
    """
...
def test_one_log_insufficient_violations():
    """
...
def test_one_log_one_violating_period(seed):
    """
...
@pytest.mark.parametrize("seed", range(10))
def test_one_log_bottom_at_zero(seed):
    """
    For a single structured log with a clear violating period above the threshold,
...
@pytest.mark.parametrize("seed", range(10))
def test_one_log_multiple_violating_periods_ignores_below_threshold(seed):
    """
    ...
    end_times = [start_times[0] + slo_s + slo_s * np.random.uniform(0, 1)] + [
...
def test_multi_log_no_violations():
    """
...
@pytest.mark.parametrize("seed", range(10))
def test_multi_log_one_violating_period(seed):
    """
...
@pytest.mark.parametrize("seed", range(10))
def test_multi_log_multiple_violating_periods(seed):
    """
    ...
    for i in range(3):
        start_times = [100 + j * 10 for j in range(5)]
        end_times = [
            s + (2 + i) * slo_s + np.random.uniform(0, 1) for s in start_times
...
@pytest.mark.parametrize("seed", range(10))
def test_multi_log_some_violate_but_not_enough(seed):
    """
...
@pytest.mark.parametrize("seed", range(10))
def test_multi_log_multiple_violating_periods_ignores_below_threshold(seed):
    """
    ...
    end_times = [s + slo_s * (2 + np.random.uniform(0, 1)) for s in start_times]
    log1 = _make_completion_structured_log(start_times, end_times)
