...
def main(args: argparse.Namespace):
    ...
    workload_name = "_".join(
        [
            "poisson",
            str(args.num_templates),
            str(args.num_query_texts_per_template),
            str(args.num_queries_per_query_text),
            str(args.poisson_lambda),
            str(args.seed),
        ]
    )
    ...
    selected_templates = all_templates[: args.num_templates]

    # Create the queries in sorted order and then shuffle.
    query_text_ids = []
    for template_idx in sorted(selected_templates):
        for query_text_idx in range(1, args.num_query_texts_per_template + 1):
            for _ in range(1, args.num_queries_per_query_text + 1):
    ...
    inter_arrival_times_s = rng.exponential(
        1 / args.poisson_lambda, size=n_queries - 1
    )
    ...
    out_path = os.path.join(pu.get_workloads_dir(), f"{workload_name}.parquet")
