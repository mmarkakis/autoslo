...
class CacheModel:
    ...
    def predict_from_tpcds_temp_and_q_idx(
        ...
        for query_id, temp_and_q_idx in query_temp_and_q_idxs.items():
            ...
            if (template_id not in self._cache) or (
                ...
                # Template cache hit
                runtimes = []
                for _, (local_runtimes, _, _) in self._cache[
                    template_id
                ].items():
    ...
    def train(
        ...
        for run_id in run_ids:
            ...
            for latency, temp_and_q_idx in zip(latencies, temp_and_q_idxs):
                template_id = Trace.extract_temp(temp_and_q_idx)
                query_within_template_id = Trace.extract_q_idx(temp_and_q_idx)

                if template_id not in self._cache:
                    self._cache[template_id] = {}
                if query_within_template_id not in self._cache[template_id]:
                    self._cache[template_id][query_within_template_id] = (
                        [],
                        0.0,
                        0.0,
                    )

                runtimes, _, _ = self._cache[template_id][
                    query_within_template_id
                ]
        ...
        all_runtimes = []
        for template_dict in self._cache.values():
            for runtimes, _, _ in template_dict.values():
                all_runtimes.extend(runtimes)
        self._overall_mean_runtime_s = float(np.mean(all_runtimes))
        self._overall_std_runtime_s = float(np.std(all_runtimes, ddof=0))
    ...
    def load(
        ...
        model._overall_std_runtime_s = params["std_overall_runtime_s"]
