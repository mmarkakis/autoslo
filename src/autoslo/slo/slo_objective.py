...
class SloObjective:
    ...
    def is_sufficient_improvement(
        self,
        baseline: ViolationCost,
        candidate: ViolationCost,
        min_rel_improvement: float,
    ) -> bool:
        """Return True if *candidate* is a meaningfully better option than
        *baseline*, combining a strict ordering check with a relative
        improvement threshold in the decisive dimension.

        Two conditions must both hold:

        1. ``cmp(candidate, baseline) == -1`` — the candidate is strictly
           better under the canonical :meth:`cmp` ordering (feasibility
           takes priority, then cost or violation depending on regime).
        2. The improvement in the *decisive dimension* is at least
           *min_rel_improvement* relative to the baseline value:

           - **Baseline infeasible → candidate feasible** (crossing the
             feasibility boundary): condition 2 is waived entirely.
             Achieving SLO compliance is unconditionally worthwhile.
           - **Both infeasible**: decisive dimension is *violation*.
           - **Both feasible**: decisive dimension is *cost*.  Do **not**
             apply a violation threshold here — when both configs are
             compliant, :meth:`cmp` already ranks purely by cost, and
             gating acceptance on violation would suppress cost-reducing
             improvements in the already-feasible regime.

        The case where baseline is feasible and candidate is infeasible
        is excluded by condition 1 (``cmp`` returns 1, not -1 in that
        case), so it is never evaluated in condition 2.
        """
        if self.cmp(candidate, baseline) != -1:
            return False

        baseline_feasible = self.is_met_from_aggregated(baseline.violation)
        candidate_feasible = self.is_met_from_aggregated(candidate.violation)

        if not baseline_feasible and candidate_feasible:
            # Crossing the feasibility boundary — no epsilon required.
            return True
        ...
        if not baseline_feasible:
            ...
            ) / max(abs(baseline.violation), 1e-9)
            return rel_improvement >= min_rel_improvement

        # Both feasible: decisive dimension is cost.
        rel_cost_improvement = (
            baseline.cost - candidate.cost
        ) / max(abs(baseline.cost), 1e-9)
        return rel_cost_improvement >= min_rel_improvement
